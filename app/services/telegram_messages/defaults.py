DEFAULT_TELEGRAM_TEMPLATES: dict[str, dict[str, str]] = {
    ...
    "progress.take_step_1": {"value": "⏳ Анализируем фото [🟩⬜⬜]\nСоздаём варианты · 1 из 3", "category": "progress", "description": "Прогресс фото: шаг 1/3 (полоска из 3 сегментов)."},
    ...
    "progress.take_parallel": {"value": "⏳ Анализируем фото [⬜⬜⬜]\nСоздаём варианты · 1 из 3", "category": "progress", "description": "Начальное состояние прогресс-бара при параллельной генерации (далее обновляется воркером: 1/3, 2/3, 3/3)."},
    "progress.suffix": {
    ...
    "errors.rate_limit": {
        "value": "⚠️ Слишком быстро\n\nВы отправили много запросов подряд.\nСделаем небольшую паузу и продолжим.\n\nПопробуйте снова после {resume_time}.",
        "category": "errors",
        "description": "Сообщение о временном ограничении частоты запросов (переменная: resume_time).",
    },
